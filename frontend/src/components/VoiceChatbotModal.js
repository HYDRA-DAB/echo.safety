      ...
      >
            ...
            <div className="flex flex-wrap gap-2">
              {currentQuickButtons.map((button, index) => (
                ...
                >
                  ...
                  {button.action.includes('navigate') && button.value.includes('map') && <Map className="w-3 h-3 mr-1" />}
                  {button.action.includes('navigate') && button.value.includes('dashboard') && <AlertTriangle className="w-3 h-3 mr-1" />}
                  {button.text}
                </Button>
              ))}
            </div>
        ...
        <div className="p-4 border-t border-gray-700/50 flex-shrink-0 bg-gray-900/80">
          ...
          <div className="text-xs text-gray-500 mt-2 text-center">
            Voice • {languagePreference === 'tamil_english' ? 'Tanglish mode' : 'English mode'} • App-aware
          </div>
        </div>
      </DialogContent>
    </Dialog>
